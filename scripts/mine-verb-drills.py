...
for ct in ["カ四", "ラ四", "ハ四", "サ四", "マ四", "カ上二", "ダ上二", "ハ下二", "ヤ下二", "ア下二", "カ上一", "マ上一", "ワ上一", "カ変", "サ変", "ナ変", "ラ変"]:
    ...
    drills.append({
        ...
        "ref_heading": "活用の種類", "sort": 200 + seq,
    })
...
for topic, pool in per_topic.items():
    ...
    for i, label in pool:
        if n >= 6: break
        key = (i["base"], i["form"])
        if not i["nxt"]: continue  # 後続語(手がかり)が無い文末はLv2に出さない
        ...
        drills.append({
            ...
            "explanation": f"{label}「{i['base']}」の{ans}。下に続く《下線》の語から活用形を判断する。（{i['title']}）",
            "ref_heading": REF.get(topic, "活用表"), "sort": 100 + n,
        })
